...
const TOP_BAR_H = 195;
const PAD = Math.max(2, Math.floor(W * 0.008)); // 自适应贴边
const BOARD_W = W - PAD*2; // 撑满宽
const BOARD_H_RAW = Math.min(Math.round(BOARD_W * 16 / 9), H - TOP_BAR_H - 130);
      ...
      if(!_nativeBtnCreated&&!userInfoBtn){
        ...
        if(sysInfo.platform!=='devtools'){try{
          ...
          userInfoBtn.onTap(function(res){
            ...
            var u=null;
            if(res.userInfo){u=res.userInfo}
            else if(res.rawData){try{u=JSON.parse(res.rawData)}catch(e){}}
            if(u&&u.nickName){
...
function handleTouch(tx,ty){
  ...
  if(showLoginOverlay){
    // 隐私协议UI — 勾选即同意
    if(!nickname){
      if(privacyCB&&tx>=privacyCB.x&&tx<=privacyCB.x+privacyCB.w&&ty>=privacyCB.y&&ty<=privacyCB.y+privacyCB.h){privacyCheckOn=!privacyCheckOn;if(privacyCheckOn){privacyAgreed=true;if(_privacyResolve){_privacyResolve({buttonId:'agree',event:'agree'});_privacyResolve=null}}else{privacyAgreed=false}return}
