...
function drawOverlays(){
  ...
  if(showLoginOverlay){
    ...
    if(!nickname){if(!userInfoBtn)showWxLoginBtn()}else{hideWxLoginBtn()}
    return;
  }
}
...
function handleTouch(tx,ty){
  ...
  // ── 登录 ──
  if(showLoginOverlay){
    if(loginCloseBB&&tx>=loginCloseBB.x&&tx<=loginCloseBB.x+loginCloseBB.w&&ty>=loginCloseBB.y&&ty<=loginCloseBB.y+loginCloseBB.h){showLoginOverlay=false;hideWxLoginBtn();return}
